...
section[data-testid="stSidebar"] {
    background: #111319 !important; /* Premium Slate hue to dramatically separate from Pure Black */
...
div[data-testid="stMetric"] [data-testid="stMetricValue"] {
    ...
    font-size: 1.15rem !important; /* Smaller font strictly ensures no truncation for values like 'Rs 804.5M' */
    white-space: normal !important;
    overflow: visible !important;
}
...
.nav-section { color: #8B949E; font-size: 0.68rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin: 24px 0 8px 0; padding: 0 4px; }
...
if page == "Overview":
    ...
    c1.metric("Facilities", fmt_val(total))
    c2.metric("Defaults", fmt_val(defaults))
    c3.metric("Act. Default %", f"{dr:.1f}%")
    c4.metric("Mean PD", f"{avg_score:.1f}%")
    c5.metric("Total ECL", fmt_val(total_el, True))
