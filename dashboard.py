...
@st.cache_resource(show_spinner="Training models & computing SHAP (one-time, ~15s)...")
def train_models():
    """Train all 3 models with same 70/30 split as notebook, compute metrics and SHAP."""
...
def page_executive_summary():
    ...
    _, _, _, _, _, all_metrics = train_models()
...
def page_explainability():
    ...
    model, explainer, shap_values, X_shap, sample_idx, all_metrics = train_models()
    ...
    render_section("Model Comparison", "3 models trained on same 70/30 split — XGBoost selected for best AUC")

    comparison_rows = ""
    for name in ["Logistic Regression", "Random Forest", "XGBoost"]:
        m = all_metrics[name]
        is_best = name == "XGBoost"
        row_bg = "rgba(59,130,246,0.08)" if is_best else "transparent"
        badge = f'<span style="color:{TH.ACCENT_BLUE}; font-size:0.6rem; font-weight:700; margin-left:8px;">SELECTED</span>' if is_best else ""
        comparison_rows += f"""
        <tr style="background: {row_bg};">
            <td style="padding:10px 12px; color:{TH.TEXT_PRIMARY}; font-weight:{'700' if is_best else '400'}; border-bottom:1px solid {TH.BORDER_LIGHT};">{name}{badge}</td>
            <td style="padding:10px 12px; color:{TH.TEXT_PRIMARY}; font-family:'JetBrains Mono',monospace; font-weight:700; border-bottom:1px solid {TH.BORDER_LIGHT};">{m['auc']:.4f}</td>
            <td style="padding:10px 12px; color:{TH.TEXT_SECONDARY}; font-family:'JetBrains Mono',monospace; border-bottom:1px solid {TH.BORDER_LIGHT};">{m['accuracy']*100:.1f}%</td>
            <td style="padding:10px 12px; color:{TH.TEXT_SECONDARY}; font-family:'JetBrains Mono',monospace; border-bottom:1px solid {TH.BORDER_LIGHT};">{m['precision']:.3f}</td>
            <td style="padding:10px 12px; color:{TH.TEXT_SECONDARY}; font-family:'JetBrains Mono',monospace; border-bottom:1px solid {TH.BORDER_LIGHT};">{m['recall']:.3f}</td>
            <td style="padding:10px 12px; color:{TH.TEXT_SECONDARY}; font-family:'JetBrains Mono',monospace; border-bottom:1px solid {TH.BORDER_LIGHT};">{m['f1']:.3f}</td>
        </tr>"""

    st.markdown(f"""
    <div style="border: 1px solid {TH.BORDER_LIGHT}; border-radius: 8px; overflow: hidden; margin-bottom: 16px;">
        <table style="width:100%; border-collapse:collapse;">
            <thead>
                <tr style="background: {TH.BG_HOVER};">
                    <th style="padding:10px 12px; text-align:left; color:{TH.TEXT_MUTED}; font-size:0.7rem; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; border-bottom:2px solid {TH.BORDER_LIGHT};">Model</th>
                    <th style="padding:10px 12px; text-align:left; color:{TH.TEXT_MUTED}; font-size:0.7rem; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; border-bottom:2px solid {TH.BORDER_LIGHT};">ROC-AUC</th>
                    <th style="padding:10px 12px; text-align:left; color:{TH.TEXT_MUTED}; font-size:0.7rem; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; border-bottom:2px solid {TH.BORDER_LIGHT};">Accuracy</th>
                    <th style="padding:10px 12px; text-align:left; color:{TH.TEXT_MUTED}; font-size:0.7rem; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; border-bottom:2px solid {TH.BORDER_LIGHT};">Precision</th>
                    <th style="padding:10px 12px; text-align:left; color:{TH.TEXT_MUTED}; font-size:0.7rem; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; border-bottom:2px solid {TH.BORDER_LIGHT};">Recall</th>
                    <th style="padding:10px 12px; text-align:left; color:{TH.TEXT_MUTED}; font-size:0.7rem; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; border-bottom:2px solid {TH.BORDER_LIGHT};">F1</th>
                </tr>
            </thead>
            <tbody>{comparison_rows}</tbody>
        </table>
    </div>
    <div style="color: {TH.TEXT_MUTED}; font-size: 0.7rem; margin-bottom: 16px;">
        Confusion Matrix (XGBoost, test set) — TN: {cm[0][0]:,} · FP: {cm[0][1]:,} · FN: {cm[1][0]:,} · TP: {cm[1][1]:,}
        &nbsp;|&nbsp; Data: Lending Club (Kaggle) · {split['train_size']:,} train · {split['test_size']:,} test · 70/30 stratified split
    </div>
    """, unsafe_allow_html=True)
