...
/* Sidebar toggle button — force visible on white backgrounds */
...
header[data-testid="stHeader"] button {{
    ...
    border: 1px solid {TH.BORDER_DARK} !important;
...
def page_executive_summary():
    ...
    <div style="background:rgba(37,99,235,0.04); border:1px solid rgba(37,99,235,0.15); border-radius:8px; padding:16px 20px; margin:4px 0 16px 0;">
        ...
        <div style="color:{TH.TEXT_SECONDARY}; font-size:0.78rem; line-height:1.7;">
            Default probability (PD) is estimated by an <strong>XGBoost</strong> gradient boosting classifier trained on 7 standardized borrower features
...
def page_advanced_analytics():
    ...
    <div style="background:rgba(37,99,235,0.04); border:1px solid rgba(37,99,235,0.15); border-radius:8px; padding:16px 20px; margin-bottom:16px;">
        ...
        <div style="color:{TH.TEXT_SECONDARY}; font-size:0.78rem; line-height:1.7;">
            Three algorithms were benchmarked: <strong>Logistic Regression</strong> (linear baseline, highly interpretable — standard in regulatory scorecards),
            ...
            XGBoost was selected for deployment based on superior <strong>AUC</strong> (primary metric for credit scoring — measures ability to rank-order risk)
            and <strong>F1 score</strong> (balances precision and recall in the presence of class imbalance: {df_master['target'].mean()*100:.1f}% default rate).
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def page_risk_predictor():
    ...
    <div style="background:rgba(37,99,235,0.04); border:1px solid rgba(37,99,235,0.15); border-radius:8px; padding:16px 20px; margin-bottom:4px;">
        ...
        <div style="color:{TH.TEXT_SECONDARY}; font-size:0.78rem; line-height:1.7;">
            Adjust borrower parameters below. The deployed <strong>XGBoost model</strong> (AUC: {all_metrics['XGBoost']['auc']:.3f}) generates a
    ...
    st.markdown(f"""
    <div style="background:{TH.BG_SECONDARY}; border:1px solid {TH.BORDER_LIGHT}; border-radius:8px; padding:12px 16px; margin-bottom:12px;">
        <div style="color:{TH.TEXT_MUTED}; font-size:0.7rem; line-height:1.6;">
            All features are <strong>standardized</strong> (scaled relative to the training data distribution).
            Negative values = below average, zero = near average, positive = above average.
            Higher values generally indicate higher risk for the borrower.
        </div>
    </div>
    """, unsafe_allow_html=True)

    # Feature inputs (all standardized)
    ...
    with col2:
        mths_since_rcnt_il = st.slider(
            ...
            help="Time since most recent installment account opened, standardized."
        )
        total_bal_il = st.slider(
            ...
            help="Total balance across all installment accounts, standardized."
        )
        il_util = st.slider(
            "Installment Utilization",
            float(df_master["il_util"].min()), float(df_master["il_util"].max()),
            float(df_master["il_util"].median()), step=0.05,
            help="Installment credit utilization ratio, standardized. Higher = more utilized."
        )
...
def main():
    ...
    <div style="background: {TH.BG_HOVER}; border-radius: 6px; padding: 12px; margin-bottom: 12px;">
        ...
        <div style="color: {TH.TEXT_SECONDARY}; font-size: 0.72rem; margin-top: 6px; line-height: 1.7;">
            XGBoost · scikit-learn · Pandas · NumPy<br>
