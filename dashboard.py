...
def load_master_dataset() -> pd.DataFrame:
    ...
    if not os.path.exists(fp):
        st.error("⚠️ CRITICAL: Master dataset not found. Verify upstream pipeline.")
        return pd.DataFrame()
...
def page_executive_summary():
    ...
    st.info(
        ...
        "Expected Credit Loss calculated using Basel III IRB approach: ECL = PD × LGD × EAD."
    )
...
def main():
    ...
    <div style="padding-bottom: 24px; border-bottom: 2px solid {TH.BORDER_LIGHT}; margin-bottom: 24px;">
        <h2 style="margin: 0; color: {TH.TEXT_PRIMARY};">📊 LENDER'S CLUB</h2>
        <p style="margin: 6px 0 0 0; color: {TH.ACCENT_BLUE}; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">
    ...
    st.sidebar.markdown(f"""
    <div style="color: {TH.TEXT_MUTED}; font-size: 0.75rem; line-height: 1.6;">
        <strong>📈 Analytics Suite</strong><br>
        Professional credit risk platform for institutional lending decision-making.
        
        <br><br><strong>⚙️ Data & Models</strong><br>
        • XGBoost PD scoring<br>
        • Basel III IRB compliant<br>
        • Real-time analytics<br>
        
        <br><strong>💼 Status</strong><br>
        ✓ System Online<br>
        ✓ Dataset: {len(df_master):,} facilities<br>
        ✓ Last Updated: Today
    </div>
    """, unsafe_allow_html=True)
