...
def page_stress_testing():
    ...
    st.markdown(f"""
<div style="background:rgba(37,99,235,0.04); border:1px solid rgba(37,99,235,0.15); border-radius:8px; padding:16px 20px; margin-bottom:16px;">
    <div style="color:{TH.ACCENT_BLUE}; font-size:0.7rem; font-weight:700; text-transform:uppercase; letter-spacing:1px; margin-bottom:6px;">
        Regulatory Context
    </div>
    <div style="color:{TH.TEXT_SECONDARY}; font-size:0.78rem; line-height:1.7;">
        Under <strong>IFRS 9</strong>, banks estimate <strong>Expected Credit Loss (ECL)</strong> to account for potential defaults in advance.
        Stress testing simulates adverse economic scenarios to assess how losses may increase under worsening conditions.
        The formula used: <strong>ECL = PD &times; LGD &times; EAD</strong>, where PD = Probability of Default, LGD = Loss Given Default, and EAD = Exposure at Default.
        <br><br>
        <em>Note: This is a simplified implementation assuming constant LGD and EAD, with stress applied via PD scaling.</em>
    </div>
</div>
""", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        migration = migration.reindex(index=RISK_ORDER, columns=RISK_ORDER, fill_value=0)
