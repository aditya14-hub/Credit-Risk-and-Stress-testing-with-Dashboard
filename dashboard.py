...
class Theme:
    ...
    TEXT_PRIMARY = "#E5E7EB"     # Crisp off-white
    TEXT_SECONDARY = "#8B94A3"   # Slate grey
    TEXT_MUTED = "#5C6575"       # Deep grey
    
    BORDER = "#1A1F2B"           # Subtle separator
    BORDER_HOVER = "#2D3748"     # Interaction border
    ...
    def get_plotly_layout(cls) -> Dict[str, Any]:
        ...
        return dict(
            ...
            xaxis=dict(
                ...
                title_font=dict(color=cls.TEXT_SECONDARY, size=11, weight="500"),
                showgrid=True,
                zeroline=False,
            ),
            yaxis=dict(
            ...
            legend=dict(
                font=dict(color=cls.TEXT_SECONDARY, size=11),
...
/* Page Headers */
.page-title {{
    font-size: 1.4rem; font-weight: 500; letter-spacing: -0.01em; color: {Theme.TEXT_PRIMARY};
}}
.status-badge {{
...
.stSelectbox label, .stMultiSelect label, .stSlider label {{
    font-size: 0.75rem !important; color: {Theme.TEXT_SECONDARY} !important; font-weight: 500 !important;
}}
.stSlider [data-testid="stThumbValue"] {{ color: {Theme.TEXT_PRIMARY} !important; }}

/* Sidebar specific */
.brand-title {{ font-size: 1rem; font-weight: 600; color: {Theme.TEXT_PRIMARY}; letter-spacing: 0.5px; }}
.brand-sub {{ font-size: 0.6rem; color: {Theme.ACCENT_MAIN}; font-weight: 500; text-transform: uppercase; letter-spacing: 2px; margin-top: 4px; }}
</style>
...
if page == "Portfolio Overview":
    ...
    with c_b2:
        ...
        fig5 = px.scatter(sample, x="prob", y="int_rate", color="risk_bucket", color_discrete_map=RISK_COLOR_MAP,
                          opacity=0.45, trendline="ols", trendline_color_override=Theme.TEXT_SECONDARY)
