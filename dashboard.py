...
def page_ml_journey():
    ...
    with tab_valid:
        ...
        _txt("<strong>Note:</strong> In production, PSI compares the <em>training period</em> against "
             "<em>recent live data</em> (e.g. this month's applicants vs. 6 months ago). "
             "Since this dataset lacks timestamps, we approximate PSI using the 70/30 train-test split. "
             "The low PSI here confirms the test set is drawn from the same distribution — a necessary "
             "sanity check — but does not substitute for time-based drift monitoring in deployment.")
