...
/* Reduce top padding and fix header overlap */
...
@st.cache_resource(show_spinner="Training XGBoost model for explainability...")
def train_xgb_model():
    """Train XGBoost on the dataset and compute SHAP values."""
...
def page_explainability():
    ...
    model, explainer, shap_values, X_shap = train_xgb_model()
    ...
    with tab1:
        ...
        # Build a bee-swarm-style strip plot
        n_sample = min(2000, len(X_shap))
        idx = np.random.RandomState(42).choice(len(X_shap), n_sample, replace=False)
        ...
        for i, col in enumerate(FEATURE_COLS):
            for j in range(n_sample):
                swarm_rows.append({
                    ...
                    "SHAP Value": shap_sample[j, i],
                    "Feature Value": X_sample.iloc[j][col]
                })
    ...
    with tab2:
        ...
        loan_shap = shap_values[loan_idx]
        base_val = explainer.expected_value
        if isinstance(base_val, np.ndarray):
            base_val = base_val[0]

        with col2:
            pred_prob = loan_row.get("prob", model.predict_proba(X_shap.iloc[[loan_idx]])[0][1])
            ...
            <div style="display:flex; gap:32px; align-items:center; padding:12px 0;">
                <div><span style="color:{TH.TEXT_MUTED}; font-size:0.75rem;">PREDICTED DEFAULT PROB</span>
                <div style="font-size:1.5rem; font-weight:700; color:{TH.TEXT_PRIMARY};">{pred_prob*100:.1f}%</div></div>
                <div><span style="color:{TH.TEXT_MUTED}; font-size:0.75rem;">RISK LEVEL</span>
                <div style="font-size:1.5rem; font-weight:700; color:{risk_color};">{risk}</div></div>
                <div><span style="color:{TH.TEXT_MUTED}; font-size:0.75rem;">ACTUAL DEFAULT</span>
        ...
        contributions = sorted(
            zip(feature_labels, loan_shap, [loan_row[c] for c in FEATURE_COLS]),
            key=lambda x: abs(x[1]), reverse=True
        )

        feat_names = [c[0] for c in contributions]
        feat_shap = [c[1] for c in contributions]
        feat_vals = [c[2] for c in contributions]
        colors = [TH.RISK_DANGER if s > 0 else TH.ACCENT_BLUE for s in feat_shap]

        fig = go.Figure(go.Bar(
            x=feat_shap, y=[f"{n} = {v:.3f}" for n, v in zip(feat_names, feat_vals)],
            orientation="h", marker_color=colors
        ))
        fig.update_layout(
            **TH.get_plotly_layout(), height=380,
            xaxis_title="SHAP Value (red = increases risk, blue = decreases)",
            yaxis_title="",
            yaxis=dict(**TH.get_plotly_layout()["yaxis"], autorange="reversed")
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        ...
        int_idx = FEATURE_COLS.index(interaction_feat)

        n_dep = min(5000, len(X_shap))
        dep_idx = np.random.RandomState(42).choice(len(X_shap), n_dep, replace=False)

        fig = px.scatter(
            x=X_shap.iloc[dep_idx][dep_feat],
            y=shap_values[dep_idx, feat_idx],
            color=X_shap.iloc[dep_idx][interaction_feat],
            color_continuous_scale=[[0, TH.ACCENT_BLUE], [1, TH.RISK_DANGER]],
            opacity=0.5,
            labels={
...
def main():
    ...
    st.sidebar.caption(
        "Real-world Lending Club loan data (30,000 loans). "
        "Features include credit grade, interest rate, utilization, "
        "and installment history. Target: actual loan default (1=Yes)."
    )

    st.sidebar.markdown("**Technology**")
    st.sidebar.caption("• XGBoost classifier\n• SHAP explainability\n• Interactive analytics")
