...
if page == "Portfolio Overview":
    ...
    with r1c1:
        ...
        fig1 = px.pie(counts, names="Tier", values="Volume", color="Tier", color_discrete_map=RISK_COLOR_MAP, hole=0.72)
        fig1.update_layout(**Theme.get_plotly_layout(), height=270, showlegend=True,
                           legend=dict(orientation="h", y=-0.05, x=0.5, xanchor="center",
                                       font=dict(color=Theme.TEXT_PRIMARY, size=11)))
        fig1.update_traces(
            textinfo="percent", textfont_size=12, textfont_color=Theme.TEXT_PRIMARY,
            marker=dict(line=dict(color=Theme.BG_CARD, width=2.5))
        )
    ...
    with r1c2:
        ...
        if "target" in fdf.columns:
            ...
            fig2 = px.bar(dr_df, x="risk_bucket", y="Rate Analysis", color="risk_bucket",
                          color_discrete_map=RISK_COLOR_MAP, text_auto=".1f")
            fig2.update_layout(**Theme.get_plotly_layout(), height=270, showlegend=False,
                               xaxis_title="", yaxis_title="Default Rate (%)")
            fig2.update_traces(textposition="outside", marker_line_width=0, opacity=0.85,
                               width=0.4,
    ...
    with r1c3:
        ...
        fig3 = px.bar(el_df, x="risk_bucket", y="ECL", color="risk_bucket",
                      color_discrete_map=RISK_COLOR_MAP, text_auto="$.2s")
        fig3.update_layout(**Theme.get_plotly_layout(), height=270, showlegend=False,
                           xaxis_title="", yaxis_title="ECL Liability ($)")
    ...
    with c_b2:
        ...
        fig5 = px.scatter(sample, x="prob", y="int_rate", color="risk_bucket",
                          color_discrete_map=RISK_COLOR_MAP, opacity=0.35)
        fig5.update_layout(**Theme.get_plotly_layout(), height=360,
                           xaxis_title="Predicted PD", yaxis_title="Nominal Interest Rate")
...
# ==============================================================================
    ...
    with c1:
        ...
        # Build layout without margin first, then override margin separately
        base_layout = Theme.get_plotly_layout()
    ...
    with c2:
        ...
        if "target" in fdf.columns:
            ...
            fig2.update_traces(
                marker_color=Theme.ACCENT_MAIN, marker_line_width=0, opacity=0.9,
                text=[f"{v:.3f}" for v in imp["Magnitude"]], textposition="outside",
                textfont=dict(color=Theme.TEXT_PRIMARY, size=11)
            )
            fig2.update_layout(**Theme.get_plotly_layout(), height=420, yaxis_title="",
                               xaxis=dict(range=[0, imp["Magnitude"].max() * 1.25]))
            st.plotly_chart(fig2, use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
